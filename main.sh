...
snatch_nh() {
	...
	for ((page=1; page<=total_pages; page++)); do
		clear
		log "INFO" "Downloading page $page of $total_pages..."
		...
		if [[ "$image_ext" == ".webp" ]]; then
			log "INFO" "Extension $image_ext detected!"
			wget --quiet --show-progress "$image_url" -O "$page.webp"
			log "INFO" "Converting $page.webp to $code/$page.png"
			dwebp -quiet "$page.webp" -o "$code/$page.png"
			rm -f "$page.webp"
			log "INFO" "Saved as $code/$page.png"
			sleep 2

		elif [[ "$image_ext" == ".jpg" || "$image_ext" == ".jpeg" || "$image_ext" == ".png" ]]; then
			wget --quiet --show-progress "$image_url" -O "$code/${page}${image_ext}"
			log "INFO" "Saved as $code/${page}${image_ext}"
			sleep 2
		fi
	done

	log "INFO" "Doujin successfully downloaded"
}
cat << "EOF"
⠄⠄⠄⢰⣧⣼⣯⠄⣸⣠⣶⣶⣦⣾⠄⠄⠄⠄⡀⠄⢀⣿⣿⠄⠄⠄⢸⡇⠄⠄
⠄⠄⠄⣾⣿⠿⠿⠶⠿⢿⣿⣿⣿⣿⣦⣤⣄⢀⡅⢠⣾⣛⡉⠄⠄⠄⠸⢀⣿⠄
